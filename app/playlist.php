...
<script>
    (function() {
        // Ensure PlaylistModule is only initialized if page exists
        if (!document.getElementById('playlistPage')) return;
        ...
        const PlaylistModule = (() => {
            ...
            function createPlaylistCard(pl) {
                ...
                div.innerHTML = `
                <div data-aos="fade-left" class="playlist-card shadow-lg p-3 d-flex justify-content-between align-items-center" style="border:1px solid #40739e;">
                ...
                fetch('playlist_actions.php', {
                    ...
                    .then(data => {
                        ...
                        if (data.status === 'success' && data.playlists.length) {
                            data.playlists.forEach(pl => container.appendChild(createPlaylistCard(pl)));
                            AOS.refresh();
                        } else {
                    ...
                    fetch('playlist_actions.php', {
                        ...
                        .then(data => {
                            if (data.status === 'success') {
                                const card = createPlaylistCard(data.playlist);
